...
public class DefaultDownloader extends Downloader{
	...
	public DownloadResult downloadClient( String clientName ) {
		...
		for ( int num = 0; num < temp.size(); num++ ) {
			...
			try {
				download( event.getCurrentFileAddress(), new File( engine.getClientBinaryFolder( clientName ), event.getCurrentFileName() ) );
		...
		File to = new File( engine.getDirectory() + File.separator + clientName + File.separator ),
			 from = engine.getClientBinaryFolder( clientName );
		...
		//TODO check stroke ArchiveManager.removeAllArchiveFiles( new File( engine.getClientBinaryFolder( clientName ), "natives") );
			
		return DownloadResult.OK;
	}
	
	private void unpack( File from, File to ) {
		String archiveWithNatives = "natives/" + engine.getPlatform().toString() + ".zip";
		
		for ( String archive : archives ) {
			if( archive.equalsIgnoreCase( archiveWithNatives ) ) {
				ArchiveManager.unpack( new File( from, archive ), new File( engine.getClientBinaryFolder( clientName ), "natives") );
			}else
				if( archive.equalsIgnoreCase( "assets.zip" ) ) {
					ArchiveManager.unpack( new File( from, archive ), new File( engine.getClientDirectory( clientName ), "assets") );
				}else
	...
	private URL getFileAddress( String filename ) {
		try {
			return new URL( NetworkManager.getServerLink( this.engine ).concat( "clients/".concat( this.clientName.concat( "/bin/".concat( filename ) ) ) ) );
