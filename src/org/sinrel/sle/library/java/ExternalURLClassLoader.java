 ...
 * @since 1.2
 */
public class ExternalURLClassLoader extends URLClassLoader implements Closeable {
    /* The search path for classes and resources */

    private final ExternalURLClassPath ucp;
     ...
     * {@code checkCreateClassLoader} method doesn't allow creation of a class
     * loader.
     * @see SecurityManager#checkCreateClassLoader
     */
    public ExternalURLClassLoader(URL[] urls, ClassLoader parent)
    {
        super(urls, parent);
        ...
        ucp = new ExternalURLClassPath(urls);
        this.acc = AccessController.getContext();
    }

    ExternalURLClassLoader(URL[] urls, ClassLoader parent, AccessControlContext acc)
    {
        super(urls, parent);
        // this is to make the stack depth consistent with 1.1
        SecurityManager security = System.getSecurityManager();
     ...
     * @see SecurityManager#checkCreateClassLoader
     */
    public ExternalURLClassLoader(URL[] urls)
    {
        super(urls);
        // this is to make the stack depth consistent with 1.1
        SecurityManager security = System.getSecurityManager();
    ...
    ExternalURLClassLoader(URL[] urls, AccessControlContext acc)
    {
        super(urls);
        // this is to make the stack depth consistent with 1.1
        SecurityManager security = System.getSecurityManager();
            ...
            URLStreamHandlerFactory factory)
    {
        ...
        ucp = new ExternalURLClassPath(urls, factory);
     ...
     * resulted in an IOException. Any such exceptions are caught internally. If
     * only one is caught, then it is re-thrown. If more than one exception is
     * caught, then the second and following exceptions are added as suppressed
     ...
     * resources.
     * <p>
     * If the URL specified is <code>null</code> or is already in the list of
     * URLs, or if this loader is closed, then invoking this method has no
     * effect.
     *
     ...
     * resource could not be found, or if the loader is closed.
     */
     ...
     * @return an <code>Enumeration</code> of <code>URL</code>s If the loader is
     * closed, the Enumeration will be empty.
     */
     ...
     */
    public static ExternalURLClassLoader newInstance(final URL[] urls, final ClassLoader parent)
    {
        // Save the caller's context
        final AccessControlContext acc = AccessController.getContext();
        // Need a privileged block to create the class loader
        ExternalURLClassLoader ucl = AccessController.doPrivileged(
                new PrivilegedAction<ExternalURLClassLoader>() {
                    public ExternalURLClassLoader run()
                    {
     ...
     * @return the resulting class loader
     */
    public static ExternalURLClassLoader newInstance(final URL[] urls)
    {
        // Save the caller's context
        final AccessControlContext acc = AccessController.getContext();
        // Need a privileged block to create the class loader
    ...
    static
    {
        ExternalSharedSecrets.seteJavaNetAccess(
                new ExternalJavaNetAccess() {
                    public ExternalURLClassPath geteURLClassPath(ExternalURLClassLoader u)
                    {
                        return u.ucp;
                    }
                }
...
final class FactoryURLClassLoader extends ExternalURLClassLoader {
